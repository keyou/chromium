...
// #pragma allow_unsafe_buffers  // Vector ++it
#endif
...
void RejectedPromises::HandlerAdded(v8::PromiseRejectMessage data) {
  ...
  // by processQueue().
  // SAFETY: We have checked it != queue_.end() and return early after earsing a
  // iterator.
  for (auto it = queue_.begin(); it != queue_.end(); UNSAFE_BUFFERS(++it)) {
